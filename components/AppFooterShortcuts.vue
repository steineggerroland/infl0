...
<template>
  <!--
    Rendered as a sibling of `<main>` by `layouts/app.vue`, so the
    `<footer>` is a page-level `contentinfo` landmark instead of a
    section footer inside `main`. We do NOT use `<Teleport to="body">`:
    Vue 3 SSR serialises teleports to `body` inside `#__nuxt` at the
    top of the tree, which made the footer render above the page
    content on reload. Keeping it inline + letting the layout decide
    the position is cheaper and survives SSR correctly.
  -->
  <footer
    :data-testid="testId"
