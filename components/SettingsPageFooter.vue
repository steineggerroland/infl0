...
const props = withDefaults(
  defineProps<{
    /** Match the settings page content column (`max-w-*`). */
    containerMax?: 'lg' | '4xl'
  }>(),
  { containerMax: 'lg' },
)
...
<template>
  <!--
    ...
    (no ClientOnly deferral).
  -->
  <Teleport to="body" defer>
    <footer
        data-testid="settings-page-footer"
        class="settings-page-footer border-t border-gray-600/80 bg-gray-400"
      >
        <div
          :class="innerMaxClass"
        ...
        >
          <nav
            class="w-full"
            :aria-label="t('settingsCommon.footerNav')"
          >
            <!--
              Global `@layer base` styles in `assets/css/tailwind.css` add
              `space-y-*` to every `ul`, which applies vertical margins between
              children and breaks a horizontal flex row (second link drifts down).
            -->
            <ul
