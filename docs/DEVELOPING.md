...
- **BDD (`npm run test:bdd`)**: user-facing feature behavior and guided journeys as the primary executable product specification. Cucumber drives **Playwright Chromium** from `features/support/world.js`. After a fresh **`npm ci`** or a Playwright version bump, install browsers once (same as E2E): `./scripts/with-nvm.sh npx playwright install chromium`. Without it, `browserType.launch` fails with “Executable doesn't exist”. The suite also raises Cucumber’s default step timeout and the **add-feed** step timeout so **`POST /api/feeds`** and cold DB responses can exceed 20 s without flaking.
